...
document.getElementById('promote-user-form').addEventListener('submit', function (e) {
    e.preventDefault();
    const userUsername = document.getElementById('promote-username').value;
    const user = users.find(u => u.username === userUsername);
    if (user) {
        user.role = 'admin';
        alert(`User "${userUsername}" promoted to admin!`);
    }
});

document.getElementById('demote-user-form').addEventListener('submit', function (e) {
    e.preventDefault();
    const userUsername = document.getElementById('demote-username').value;
    const user = users.find(u => u.username === userUsername);
    if (user) {
        user.role = 'user';
        alert(`User "${userUsername}" demoted from admin!`);
    }
});
